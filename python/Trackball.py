...
class Trackball:
    ...
    # Create the root window
        ...
        # Checks for space press to switch self.state
        ...
        self.dirname = tk.Button(
            self.frame, text='Select directory', command=self.choose_dir)
        ...
        self.mouse = tk.Radiobutton(
            self.frame, text='Mouse', value='mouse', variable=self.selected)
        self.mouse.grid(row=2, column=1, pady=5, padx=5, sticky="w")
        self.of_sensor = tk.Radiobutton(
            self.frame, text='OF Sensors', value='opticflow', variable=self.selected)
        ...
        self.start = tk.Button(
            self.frame, text='Start (stop with SPACE)', command=self.experiment_loop)
    ...
    def output_csv(self):
        df = pd.DataFrame(self.moves, columns=(
            'phi', 'dist', 'x', 'y', 'time'))
        df.to_csv(self.input + '/' + self.animal_id + '-%s.csv' % self.dt)

    def mouse_exp(self):
        # Date and time of the beginning of experiment
        ...
        try:
            while self.state == True:
                ...
                mouse.move(self.pos_init[0, 0],
                           self.pos_init[0, 1], absolute=True)
    ...
    def update_data(self):
        # np.array([mousepos.x, mousepos.y])
        mousepos_abs = np.asarray(mouse.get_position())
        ...
        self.pos_change = math.sin(self.phi_ang) * \
            dist, math.cos(self.phi_ang)*dist
        self.new_pos = self.new_pos + self.pos_change
        curr_time = time.time() - self.t0
        mousepos_timed = np.array(
            [self.phi_ang, dist, self.new_pos[0], self.new_pos[1], curr_time])
