...
class SerialWindow():
    def __init__(self, labels, coordinates, toplevel=None):
        ...
        self.x, self.y = coordinates[2] + coordinates[0] + 10, coordinates[3] + 250 + self.pad
        ...
        for ser in self.working_serial:
            ...
            lambda_toggle_serial = lambda x = ser_n: (
                self.toggle_serial(
                    ser_n
                )
            )
            ...
            self.dropdown_serial[ser_n] = ttk.Combobox(self.window_serial,
                state="readonly",
                justify=tk.CENTER,
                width=16,
                values=self.working_serial)
            self.dropdown_serial[ser_n].current(ser_n)
            self.button_open_serial[ser_n] = tk.Button(self.window_serial, 
                text=self.labels["t_serial_open"],
                width=15,
                command=lambda_toggle_serial
            )
            self.dropdown_scripts[ser_n] = ttk.Combobox(self.window_serial,
                state="readonly",
                justify=tk.CENTER,
                width=16,
                values=None
                )
            self.text_command[ser_n] = tk.Entry(self.window_serial,
                width=17
            ) # Include example text: https://stackoverflow.com/questions/51781651/showing-a-greyed-out-default-text-in-a-tk-entry
            self.button_send_serial[ser_n] = tk.Button(self.window_serial, 
